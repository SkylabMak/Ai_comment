...
<div class={`relative z-10 ${s}`} aria-labelledby="modal-title" role="dialog" aria-modal="true" tabindex="0" on:click={(event) => {
    ...
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div class="flex min-h-full items-end p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-64 modal-content h-auto" on:click={(e) => e.stopPropagation()} style="transform: translate(1040px, -174px);">
                <div class="bg-white px-0 pb-0 pt-0 sm:p-0">
                    {#each emotions as checked, index}
                    <div class={`flex items-center mb-0 p-3 ml-0 ${checked ? 'bg-[#F4E9FE] text-[#6E0BD4]' : 'border-white'}`}>
                        <input
