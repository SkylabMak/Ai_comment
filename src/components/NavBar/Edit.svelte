...
<div class={`relative z-10 ${e}`} aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto backdrop-blur-sm bg-opacity-50">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-[1000px] modal-content">
                <class class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 mt-2">
                    <button class="absolute top-3 right-5" on:click={() => { e = "hidden"; }}>x</button>
                    <div class="ml-6 mr-6 w-full">
                        ...
                        <div class="w-full">
                            ...
                            <div>
                                ...
                                <div class="flex items-center justify-between mb-2 border border-[#E6E9F4] p-6 rounded w-[955px]">
                                    ...
                                    <button class="bg-white rounded-full p-2 ml-10" on:click={() => openRenameModal(product.Product_ID)}>
                                        <img src="/src/lib/images/EditPencil.png" alt="EditPencil" class="h-5 w-5 ml-2 mr-8" />
                                    </button>

                                    <!-- Rename Modal -->
                                    {#if r === "block" && selectedProductId === product.Product_ID}
                                    <div class="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm bg-opacity-50">
                                        <Rename cKey={cKey} id={selectedProductId} bind:r={r} />
                                    </div>
                                    {/if}
                                </div>
                                {/each}
