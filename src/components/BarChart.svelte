<script lang="ts">
	...
	alldataComment.subscribe((value) => {
		...
		if (allComment && allComment.length > 0) {
			...
			// If allComment is empty, set isDataLoaded to false and clear chartData
			isDataLoaded = false;
			chartData = null;
		}
	});
	...
	function handleFilterChange(e: Event) {
		...
		if (target) {
			timeFilter.set(target.value);
		}
	}
</script>

<div class="relative pt-4 flex flex-col flex-grow">
	<!-- Dropdown for selecting the time filter -->
	<select
		on:change={(e) => {
			handleFilterChange(e);
		}}
		class="absolute top-[-16px] right-0 p-2 border rounded"
	>
	...
	{#if isDataLoaded && chartData}
		...
		<div class="relative 2xl:h-[400px] lg:h-[200px] w-full">
			<Bar
				data={chartData}
				options={{
					responsive: true,
